...
class GloVe:
    ...
    def _load_vectors(self):
        ...
        if self.name == "twitter.27B":
            filepaths.append(os.path.join(self.cache, f"glove.twitter.27B.{self.dim}d.txt"))

        filepath = next((p for p in filepaths if os.path.exists(p)), None)
        if not filepath:
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                try:
                    ...
                    word = values[0]
                    vector = np.array([float(v) for v in values[1:]], dtype=np.float32)
    ...
    def __getitem__(self, token):
        ...
        if token in self.stoi:
            return torch.tensor(self.vectors[self.stoi[token]], dtype=torch.float)
